...
const Sidebar = () => {
    ...
    const [userInfo, setUserInfo] = useState({ Username: '', ProfilePicture: '', UserCode: '' });
    ...
    useEffect(() => {
      // Ajusta si la barra lateral es visible según el tamaño de la pantalla
      const handleResize = () => {
          setIsSidebarVisible(window.innerWidth > 768);
      };

      // Llama a la función en el montaje del componente
      handleResize();
    ...
    useEffect(() => {
        const cargarSalas = async () => {
            ...
            if (userId) {
                try {
                    const salasResponse = await axios.get(`/api/user-rooms/${userId}`);
    ...
    const renderProfilePicture = () => {
      // Si DisplayName no está disponible, mostrar "Usuario#UserCode"
      const userDisplayName = userInfo.DisplayName || (
        <>
          Usuario
          <span className="user-code">#{userInfo.UserCode || '0000'}</span>
        </>
      );
      const initial = userInfo.DisplayName ? userInfo.DisplayName[0].toUpperCase() : 'U';
      ...
      return (
        <>
          ...
          <div className="user-info">
            <span className="user-nick">{userDisplayName}</span>
    ...
    const submitRename = async (roomCode) => {
        ...
        try {
          const response = await axios.put(`/api/rename-room/${roomCode}`, { customRoomName: customName });
    ...
    const esconderSala = async (roomCode) => {
        ...
        try {
            ...
            await axios.delete(`/api/remove-room-from-list/${roomCode}`);
          ...
          <div className="sidebar-content">
            <div className="title-with-button">
              <button onClick={() => navigate('/create-room')} className="icon-button">
                <FontAwesomeIcon icon={faPlus} style={{ color: '#2a4857' }} />
              </button>
              <h3>Mis Salas</h3>
            </div>
